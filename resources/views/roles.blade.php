                ...
                <td class="text-center">
                  <div class="actions">
					...
					<button 
							...
							class="btn btn-sm bg-success-light editbtn">
							<i class="fe fe-pencil"></i> Edit
						</button>
...
<!-- Bootstrap JS (make sure this matches the version you're using for the CSS) -->
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.bundle.min.js"></script>
  <script>
    ...
    $(document).on('click', '.editbtn', function() {
        var id = $(this).data('id');
        var role = $(this).data('role');
        var permissions = $(this).data('permissions'); // Permissions passed as a JSON string

        // Check if id, role, and permissions are properly set
        if (!id || !role || !permissions) {
            console.error("Missing data-id, data-role, or data-permissions");
            return; // Exit if any of the data attributes are missing
        }

        // Parse permissions JSON safely
        var selectedPermissions = permissions; // No need to parse, it’s already a JS object/array

        // Set form values
        $('#edit_id').val(id);
        $('.edit_role').val(role);
        $('.edit_perms').val(selectedPermissions).trigger('change');
